        ...
        public PersonMapper()
        {
           ...
           .ForMember(dest => dest.RefOtherNameTypeId, map => map.MapFrom(src => src.otherNameList.Any() ? CommonMethods.GetCodesetCode("RefOtherNameType", "RefOtherNameTypeId", src.otherNameList.FirstOrDefault().otherName.preferredGivenName) : null));
            ...
            CreateMap<Person,PersonStatus>()
                .ForMember(dest => dest.RefPersonStatusTypeId, map => map.MapFrom(src => src.demographics.languageProficiency.ToString().ToLower() == "yes" ? CommonMethods.GetCodesetCode("RefPersonstatustype", "RefPersonStatusTypeId", MyEnumClass.LEPStatus) : null));
