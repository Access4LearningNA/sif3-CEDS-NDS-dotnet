...
namespace Sif.NdsProvider.Mappers
{
    public class AddressMapper:Profile
    {
        ...
        {
            Mapper.Initialize(cfg => cfg.CreateMap<Address, OrganizationLocation>()
            .ForMember(dest => dest.RefOrganizationLocationTypeId, map => map.MapFrom(src => src.addressType.code.ToString() !=null? CommonMethods.GetCodesetCode("RefOrganizationLocationType", "RefOrganizationLocationTypeId", src.addressType.ToString()) : null))
             .ForMember(dest => dest.RefOrganizationLocationTypeId, map => map.MapFrom(src => src.addressType.otherCodeList.Select(x=>x.Value.ToString() != null ? CommonMethods.GetCodesetCode("RefOrganizationLocationType", "RefOrganizationLocationTypeId", src.addressType.ToString()) : null))));
            Mapper.Initialize(cfg => cfg.CreateMap<Address, PersonAddress>()
            ...
            Mapper.Initialize(cfg => cfg.CreateMap<Address, LocationAddress>()
