    ...
    public class LeaMapper : Profile
    {
        ...
        public LeaMapper()
        {
            ...
            CreateMap<Lea, PersonTelephone>()
                ...
                .ForMember(dest => dest.PrimaryTelephoneNumberIndicator, map => map.MapFrom(src => src.leaContactList.Select(x => x.phoneNumberList.Any(y => y.listedStatus.ToString() == "Yes"))))
                .ForMember(dest => dest.RefPersonTelephoneNumberTypeId, map => map.MapFrom(src => src.leaContactList.Select(x => x.phoneNumberList.Select(y => y.phoneNumberType.code.ToString()) != null ? CommonMethods.GetCodesetCode("refpersontelephonenumbertype", "RefPersonTelephoneNumberTypeId", "Code", x.phoneNumberList.Select(y => y.phoneNumberType.code.ToString()).FirstOrDefault()) : null)));
            CreateMap<Lea, PersonEmailAddress>()
                ...
                .ForMember(dest => dest.RefEmailTypeId, map => map.MapFrom(src => src.leaContactList.Select(x => x.emailList.Select(y => y.emailType.code.ToString()) != null ? CommonMethods.GetCodesetCode("refemailtype", "RefEmailTypeId", "Code", x.emailList.Select(y => y.emailType.code.ToString()).FirstOrDefault()) : null)));
            CreateMap<Lea, PersonOtherName>()
                .ForMember(dest => dest.RefOtherNameTypeId, map => map.MapFrom(src => src.leaContactList.Select(x => x.otherNameList.Select(y => y.otherName.preferredFamilyName.ToString() != null ? CommonMethods.GetCodesetCode("refothernametype", "RefOtherNameTypeId", "Code", y.otherName.preferredFamilyName.ToString()) : null))))
                .ForMember(dest => dest.RefOtherNameTypeId, map => map.MapFrom(src => src.leaContactList.Select(x => x.otherNameList.Select(y => y.otherName.preferredGivenName.ToString() != null ? CommonMethods.GetCodesetCode("refothernametype", "RefOtherNameTypeId", "Code", y.otherName.preferredGivenName.ToString()) : null))))
